...
const App = () => {
  ...
  const url = `https://api.openweathermap.org/data/2.5/weather?q=${location}&appid=fd381ddb7a2ba6bfbf705f6fd15bd5dd&units=metric`
    ...
    // const mystringArray =["clear sky","overcast", "overcast clouds","light rain","mist"];
  ...
  data.weather && data.weather.length > 0 && getSource(data.weather[0].description) && 
    <video autoPlay='autoplay' muted loop>
      ...
      <div className="search">
        ...
        type="text"
        value={location}
        // onchange occurs whenvalue of the element has changed.
      ...
      <div className="top">
        ...
        <div>
          {data.weather ? <p>{capitalName(data.weather[0].description)}</p> : null}
        </div>
        <div>
        {data.main ? <p>L:{data.main.temp_min.toFixed()}° H:{data.main.temp_max.toFixed()}°</p> : null}
        </div>
